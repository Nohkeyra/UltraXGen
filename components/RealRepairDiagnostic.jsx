...
export const RealRepairDiagnostic = ({ onComplete }) => {
  const [nodes, setNodes] = useState([]);
  const [isScanning, setIsScanning] = useState(true);
  const [logs, setLogs] = useState(null);

  const addLog = useCallback((msg, type: 'info' | 'success' | 'error' = 'info') => {
    setLogs(prev => [{
      message,
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const initialNodes = subsystems.map((name, i) => ({
        id,
        subsystem,
        integrity) * 35,
        status,
        repaired,
        isRepairing,
        address) * 0xFFFFFF).toString(16).toUpperCase().padStart(6, '0')}`
      }));

      setNodes(initialNodes);
      setIsScanning(false);
      addLog(`AUDIT_COMPLETE, 'error');
    };
    init();
  }, [addLog]);
  ...
  const handleRepair = async (id) => {
    ...
    setNodes(prev => prev.map(n => n.id === id ? { ...n, isRepairing));
    addLog(`PATCH_SEQUENCE_INIT, 'info');
    
    await new Promise(r => setTimeout(r, 1500));
    
    setNodes(prev => {
      const updated = prev.map(n => n.id === id ? { ...n, integrity, status, repaired, isRepairing);
  ...
  return (
    <div className="fixed inset-0 z-[600] bg-brandNeutral dark:bg-brandDeep flex flex-col font-mono overflow-hidden animate-in fade-in duration-500">
          ...
          <div className="flex items-center gap-3">
             <div className="flex flex-col">
               ...
               <div className="flex items-center gap-2">
                 <div className={`w-1.5 h-1.5 rounded-full ${allRepaired ? 'bg-brandYellow' : 'bg-brandYellow animate-pulse'}`} />
                 <span className={`text-xs font-black italic ${allRepaired ? 'text-brandYellow' : 'text-brandYellow'}`}>{Math.round(repairProgress)}%_OK</span>
               </div>
             </div>
          </div>
        </div>
        ...
        <div className="flex gap-4">
           {!allRepaired && (
             <button 
               ...
               className="bg-brandRed/10 border border-brandRed text-brandRed px-6 py-2 text-[10px] font-black uppercase tracking-widest hover,0,1,0.2)]"
             >
               Bulk_Restore
             </button>
           )}
           <button 
             ...
             className={`px-6 py-2 text-[10px] font-black uppercase tracking-widest transition-all ${allRepaired ? 'bg-brandYellow text-brandBlue hover:bg-opacity-90' : 'bg-white/5 border border-white/10 text-white/40 hover:text-brandRed'}`}
           >
            ...
            {nodes.map(node => (
              <div 
                ...
                className={`p-5 border-2 transition-all relative group overflow-hidden ${
                  node.repaired 
                    ? 'bg-brandYellow/5 border-brandYellow/30' 
              ...
              >
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className={`text-[8px] px-2 py-0.5 rounded-sm font-black border ${
                    node.status === 'STABLE' ? 'bg-brandYellow/20 text-brandYellow border-brandYellow/30' : 
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-[9px] font-black uppercase">
                    ...
                    <span className={node.repaired ? 'text-brandYellow' : 'text-brandYellow'}>{Math.round(node.integrity)}%</span>
                  </div>
                  <div className="h-1 bg-white/5 w-full rounded-full overflow-hidden">
                    <div 
                      className={`h-full transition-all duration-1000 ${node.repaired ? 'bg-brandYellow' : 'bg-brandRed'}`}
                ...
                {!node.repaired && (
                  <button 
                    ...
                    className="mt-4 w-full py-2 bg-white/5 border border-white/10 text-white/60 hover)}
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="p-6 border-b border-white/5">
            ...
            <div className="h-64 overflow-y-auto custom-scrollbar space-y-2 font-mono text-[9px]">
              {logs.length === 0 ? (
                ...
                logs.map((log, i) => (
                  <div key={i} className={`flex gap-3 animate-in slide-in-from-left duration-300 ${log.type === 'error' ? 'text-brandRed' : log.type === 'success' ? 'text-brandYellow' : 'text-white'}`}>
      ...
      <div className="h-1 bg-white/5">
        <div 
          className="h-full bg-brandRed transition-all duration-1000 shadow-[0_0_10px_#CC0001]"
          style={{ width);
};
