...
export const SystemAuditPanel = () => {
  ...
  const metrics = [
    { label)', score, status, description,
    { label)', score, status, description,
    { label)', score, status, description,
    { label)', score, status, description) => {
            ...
            {metrics.map((m, i) => (
              <div key={i} className="bg-brandCharcoal dark:bg-black/40 border border-white/5 p-6 rounded-sm group hover:border-brandRed transition-all">
                <div className="flex justify-between items-end mb-4">
                  ...
                  <div className={`text-2xl font-black italic ${m.status === 'CRITICAL' ? 'text-brandRed' : m.status === 'WARNING' ? 'text-brandYellow' : 'text-brandYellow'}`}>
                    {m.score}%
                  </div>
                </div>
                <div className="h-1 bg-white/5 w-full mb-4">
                  <div 
                    className={`h-full transition-all duration-1000 ${m.status === 'CRITICAL' ? 'bg-brandRed' : m.status === 'WARNING' ? 'bg-brandYellow' : 'bg-brandYellow'}`}
                    style={{ width: `${m.score}%` }}
                  />
                </div>
                <p className="text-[9px] font-bold text-brandCharcoalSoft dark))}
          </div>
